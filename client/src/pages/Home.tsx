...
function BrandMatrix({ activeBrandId }: { activeBrandId: string }) {
    ...
    <section className="brand-workspace">
      ...
      <div className="matrix-shell">
        ...
        <div className="matrix-table" role="table">
          <div className="matrix-row matrix-head"><div className="matrix-label-cell">比較項目</div>{brands.map((brand) => <div className={`matrix-brand-cell ${brand.id === "ricoh" ? "matrix-ricoh" : ""} ${brand.id === activeBrandId ? "matrix-brand-active" : ""}`} data-brand-id={brand.id} key={brand.id}><span className="brand-index">{brand.index}</span><strong>{brand.name}</strong><Stamp tone={brand.statusTone}>{brand.regionStatus}</Stamp><small>查核 {brand.checkedAt}</small></div>)}</div>
          {groups.map((group) => openGroups[group.id as keyof typeof openGroups] && <div key={group.id} className="matrix-group"><button className="matrix-group-title" onClick={() => setOpenGroups({ ...openGroups, [group.id]: !openGroups[group.id as keyof typeof openGroups] })}><span><span className="group-rule" />{group.label}</span><ChevronDown size={15} /></button>{rows.slice(group.start, group.end).map(([key, label]) => <div className="matrix-row" key={key}><div className="matrix-label-cell"><span>{label}</span></div>{brands.map((brand) => { const field = brand.fields[key]; return <div className={`matrix-value-cell ${brand.id === "ricoh" ? "matrix-ricoh" : ""} ${brand.id === activeBrandId ? "matrix-brand-active" : ""}`} key={`${brand.id}-${key}`}><span>{field?.value ?? "待建檔"}</span>{field?.status && <span className={`cell-flag ${field.status === "待確認" ? "flag-blue" : "flag-amber"}`}>{field.status}</span>}{field?.sourceUrl && <a className="cell-evidence" href={field.sourceUrl} target="_blank" rel="noreferrer">來源 <ExternalLink size={11} /></a>}</div>; })}</div>)}</div>)}
        </div>
      </div>
...
export default function Home() {
  ...
  const [activeBrandId, setActiveBrandId] = useState("ricoh");
  ...
  const selectBrand = (brandId: string) => {
    ...
    window.requestAnimationFrame(() => {
      document.querySelector(`[data-brand-id="${brandId}"]`)?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "center" });
    });
  };
